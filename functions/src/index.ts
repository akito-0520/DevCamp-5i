...
const calculateDistance = (pos1: {x: number, y: number}, pos2: {
  x: number,
  y: number
...
export const onHackathonDeadline =
    onDocumentUpdated("hackathon/{hackathonId}", async (event) => {
      ...
      if (dataBefore?.is_deadline === false &&
          dataAfter?.is_deadline === true) {
        console.log(
        ...
        try {
          ...
          const snapshot =
              await tasksCollectionRef.where("hackathon_id", "==", hackathonId)
          ...
          const within =
            ...
            (teamSizeUpper !== null && teamSizeLower === null && acceptCount <= teamSizeUpper) ||
            (teamSizeLower !== null && teamSizeUpper === null && acceptCount >= teamSizeLower) ||
            (teamSizeLower !== null && teamSizeUpper !== null && acceptCount >= teamSizeLower && acceptCount <= teamSizeUpper);
          if (within) {
            // 参加人数が適切な場合
            // 1. バッチ処理を初期化
            console.log("人数は適切");
          ...
          if (acceptCount > targetTeamSize) {
            ...
            const currentUserGroupInfoSnap =
                await db.collection("group_list")
            ...
            for (const member of participantsWithDistance) {
              if (selectedMembers.length >= targetTeamSize)
                break;
